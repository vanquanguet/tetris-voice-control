...
class Tetris(object):
    ...
    def apply_action(self):
        ...
        action.
        """
        dic_index2 = {'t':'left','p':'right', 'x':'rotate', 'o': 'drop', '' : 'none'}
        ...
        if command != '':
            command = command[-1]
            print(dic_index2[command])
            f2 = open('command.txt','w')
            f2.write('')
            f2.close()
        if command == 't':
...
if __name__ == "__main__":
    thread1 = threading.Thread(target=run_game, args=())
    thread2 = threading.Thread(target=run_controller, args=())
